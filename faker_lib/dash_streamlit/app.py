...
try:
    ...
    st.error("Arquivo 'vendas_eletronicos.csv' não encontrado. Por favor, verifique se o arquivo está no mesmo diretório do script.")
    st.stop()
...
if df_filtrado.empty:
    ...
    with col1:
        ...
        top_produtos_valor = df_filtrado.groupby('Produto')['Total_Venda'].sum().nlargest(5).reset_index()
    ...
    with col2:
        ...
        top_produtos_qtd = df_filtrado.groupby('Produto')['Quantidade'].sum().nlargest(5).reset_index()
        fig_prod_qtd = px.bar(
            top_produtos_qtd,
            x='Quantidade',
            y='Produto',
    ...
    vendas_por_mes = df_filtrado.groupby(['Ano', 'Mês'])['Total_Venda'].sum().reset_index()
    vendas_por_mes['Data'] = vendas_por_mes['Ano'].astype(str) + '-' + vendas_por_mes['Mês'].astype(str)
    fig_mes = px.bar(
        vendas_por_mes,
        x='Data',
        y='Total_Venda',
    ...
    top_clientes_vendas = df_filtrado.groupby('Nome_Cliente')['Total_Venda'].sum().nlargest(5).reset_index()
    fig_clientes = px.bar(
        top_clientes_vendas,
        x='Total_Venda',
        y='Nome_Cliente',
